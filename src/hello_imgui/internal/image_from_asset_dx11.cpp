...
{
    static std::unordered_map<std::string, ImageDx11Ptr> gImageFromAssetMap;


    static ImageDx11Ptr _GetCachedImageDx11(const char*assetPath)
    {
        ...
        if (gImageFromAssetMap.find(assetPath) == gImageFromAssetMap.end())
            gImageFromAssetMap[assetPath] = std::make_shared<ImageDx11>(width, height, image_data_rgba);
    ...
    {
        auto cachedImage = _GetCachedImageDx11(assetPath);
        return (ImTextureID) cachedImage->ShaderResourceView;
    }
