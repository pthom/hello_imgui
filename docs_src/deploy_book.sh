...
if [[ -n $(git status -s --untracked-files=no) ]]; then
    echo "ERROR: ******** Please commit all changes before deploying ********"
...
if [[ $? -ne 0 ]]; then
    echo "ERROR: ******** Error when merging master into docs branch. Check for merge conflicts. ********"
    exit 1
fi
...
if [[ ! -f "$REPO_DIR/venv/bin/python" ]]; then
    echo "ERROR: ******** Please create the virtual environment first ********"
    exit 1
fi
...
if [[ $? -ne 0 ]]; then
    echo "ERROR: ******** Error building docs ********"
    exit 1
fi
...
git checkout master
if [[ $? -ne 0 ]]; then
    echo "ERROR: ******** Error checking out master branch ********"
    exit 1
fi
